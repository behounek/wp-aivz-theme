...
$similarPostsPortfolio = get_option ( THEME_NAME."_similar_posts_portfolio" ); 
$homeID = get_home_page();
$galleryID = df_get_page('gallery');

$fullID = get_fullwidth_page();
$portfolioID = get_portfolio_page();
...
//post settings
$box_array[] = array( 'id' => 'post-title', 'title' => ''.$image.__("Show Title/Date/Author/Category", THEME_NAME), 'page' => 'post', 'context' => 'normal', 'priority' => 'high', 'fields' => array(array('name' => __("Show Title:", THEME_NAME), 'std' => '', 'id' => $prefix. 'show_title', 'type'=> 'show_hide' ) ), 'size' => 10, 'first' => 'yes' );
$box_array[] = array( 'id' => 'post-swicher', 'title' => ''.$image.__("Show Prev/Next Buttons", THEME_NAME), 'page' => 'post', 'context' => 'normal', 'priority' => 'high', 'fields' => array(array('name' => __("Show:", THEME_NAME), 'std' => '', 'id' => $prefix. 'post_btn', 'type'=> 'yes_no' ) ), 'size' => 10, 'first' => 'no' );
$box_array[] = array( 'id' => 'fancybox', 'title' => ''.$image.__("Enable Fancy Box On Blog Post Images", THEME_NAME), 'page' => 'post', 'context' => 'normal', 'priority' => 'high', 'fields' => array(array('name' => __("Fancybox:", THEME_NAME), 'std' => '', 'id' => $prefix. 'fancybox', 'type'=> 'on_off' ) ), 'size' => 10,'first' => 'no'  );
$box_array[] = array( 'id' => 'post-type', 'title' => ''.$image.__("Post Type", THEME_NAME), 'page' => 'post', 'context' => 'normal', 'priority' => 'high', 'fields' => array(array('name' => __("Type:", THEME_NAME), 'std' => '', 'id' => $prefix. 'post_type', 'type'=> 'post_type' ) ), 'size' => 10,'first' => 'no'  );
$box_array[] = array( 'id' => 'video-url', 'title' => ''.$image.__("Vimeo/YouTube Video URL", THEME_NAME), 'page' => 'post', 'context' => 'normal', 'priority' => 'high', 'fields' => array(array('name' => __("Url:", THEME_NAME), 'std' => '', 'id' => $prefix. 'video', 'type'=> 'text' ) ), 'size' => 10,'first' => 'no'  );
...
$box_array[] = array( 'id' => 'main-slider-post', 'title' => ''.$image.__("Main Slider Post", THEME_NAME), 'page' => 'post', 'context' => 'normal', 'priority' => 'high', 'fields' => array(array('name' => __("Show", THEME_NAME), 'std' => '', 'id' => $prefix. 'main_slider', 'type'=> 'no_yes' ) ), 'size' => 10,'first' => 'no'  );
...
if($currentID==get_option('page_for_posts') || isset($_POST['post_type'])) {
	$box_array[] = array( 'id' => 'color-page', 'title' => ''.$image.' Page Title Color In Homepage Blocks', 'page' => 'page', 'context' => 'normal', 'priority' => 'high', 'fields' => array(array('name' => 'Page Color:', 'std' => get_option(THEME_NAME."_default_cat_color"), 'id' => $prefix. 'title_color', 'type'=> 'color' ) ), 'size' => 10, 'first' => 'yes' );
}


// post ratings
$box_array[] = array('id' => 'post-ratings', 'title' => ''.$image.__(" Ratings", THEME_NAME),'page' => 'post','context' => 'normal','priority' => 'high','fields' => array(array('name' => __("Enter the ratings like this - <b>Illustration:5; Design:4,5; <br>Photography:3; Interface:4; Quality:4</b>", THEME_NAME),'std' => '','id' => $prefix. 'ratings','type'=> 'textarea')),'size' => 10,'first' => 'no' );
...
//share buttons
if($shareAll=="custom" && !in_array($currentID, $contactID) && $currentID!=get_option('page_for_posts') && !in_array($currentID, $galleryID) && !in_array($currentID, $homeID) && !in_array($currentID, $portfolioID) || isset($_POST['post_type'])) {
	...
	$box_array[] = array( 'id' => 'share-page', 'title' => ''.$image.' Show Share Buttons', 'page' => 'page', 'context' => 'normal', 'priority' => 'high', 'fields' => array(array('name' => 'Show Share Buttons:', 'std' => 'hide', 'id' => $prefix. 'share_single', 'type'=> 'show_hide' ) ), 'size' => 10, 'first' => 'no' );
}
...
//images
if($showSingleThumb=="on" && !in_array($currentID, $fullID) && !in_array($currentID, $homeID) && !in_array($currentID, $galleryID) && !in_array($currentID, $contactID) || isset($_POST['post_type']) || $currentID==0) {
...
if($aboutAuthor=="custom") {
	//$box_array[] = array( 'id' => 'about-author-post', 'title' => ''.$image.' About Author', 'page' => 'post', 'context' => 'normal', 'priority' => 'high', 'fields' => array(array('name' => 'About Author:', 'std' => '', 'id' => $prefix. 'about_author', 'type'=> 'hide_show' ) ), 'size' => 10, 'first' => 'no' );
}
...
if(in_array($currentID, $homeID) || isset($_POST['post_type'])) {
	//home slider
	$box_array[] = array( 'id' => 'home-slider', 'title' => ''.$image.__("Homepage Slider", THEME_NAME), 'page' => 'page', 'context' => 'normal', 'priority' => 'high', 'fields' => array(array('name' => __("Option", THEME_NAME), 'std' => '', 'id' => $prefix. 'slider_type', 'type'=> 'slider_type' ) ), 'size' => 10,'first' => 'no'  );
	$box_array[] = array( 'id' => 'intro-text', 'title' => ''.$image.__("Homepage Intro Text (Text/HTML/Shortcode)", THEME_NAME), 'page' => 'page', 'context' => 'normal', 'priority' => 'high', 'fields' => array(array('name' => __("Text", THEME_NAME), 'std' => '', 'id' => $prefix. 'header_text', 'type'=> 'textarea' ) ), 'size' => 10,'first' => 'no'  );
...
function show_box_funtion($fields, $first='no', $width='60') {
	...
	foreach ($fields['fields'] as $field) {
		...
		switch ($field['type']) {
			...
			case 'post_type':
				$value = array('Default', 'Video', 'Image', 'Music', 'Off');
