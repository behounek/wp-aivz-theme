	...
	*/
	function print_homepagesettest($value){
		echo $this->before_item.$this->before_item_title.$value['title'].$this->after_item_title;
						...
						if($homepage == "page" && ($meta[0] == "template-homepage.php" || $meta[0] == "template-homepage-style-2.php" || $meta[0] == "template-homepage-style-3.php" || $meta[0] == "template-homepage-style-4.php" || $meta[0] == "template-homepage-style-5.php" ) ) { global $has_homepage; $has_homepage=true; } else { $has_homepage=false; }
			...
			echo $this->before_item.$this->before_item_title.$value['title'].$this->after_item_title.'
					<ul class="blocks block-available clearfix inactive-blocks" id="available-homepage-blocks">';
						...
						<li class="inactive-block component" rel="'.$block['type'].'">
							<div class="blocks-content clearfix">'.$block['title'].'<a href="javascript:{}" class="button move">Move</a>
					...
					<?php
						if($google_list!=false) {
							foreach($font_list as $key => $font) {
								...
								if($key==0) {
									echo '<option value="'.$font['font'].'">'.$font['font'].' '.$font['txt'].'</option>';
								} else {
									echo '<option value="'.$font.'" '.$selected.'>'.$font.'</option>';
								}
							}
						} else {
							echo '<option value="">Something is Wrong with your server configuration.</option>';
						}
					?>
				</select>
			</span>
